...
for file in bin/holonight-viewer share/applications/org.holonight.Viewer.desktop \
...
cd "$stage"
QT_QPA_PLATFORM=offscreen "$stage/usr/bin/holonight-viewer" --version
set +e
QT_QPA_PLATFORM=offscreen QSG_RHI_BACKEND=software timeout 3 "$stage/usr/bin/holonight-viewer" > runtime.log 2>&1
status=$?
set -e
cat runtime.log
test "$status" -eq 124

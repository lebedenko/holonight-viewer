...
| P7 | REQ-F-024 vs REQ-F-011 | Notice must stay until navigation, but the strip conceals after 4 s. | Notice is part of the strip and re-appears on every reveal; the strip is revealed on failure and the event is announced for accessibility. Add a pinned label only if the user wants persistence beyond reveals. |
